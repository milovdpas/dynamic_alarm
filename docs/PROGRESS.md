...
| 2026-08-19 | **Only something that made a request may report a live connection.** `writeCache` ended with `markLive()`, so `useScheduleBundle` and `useApiQuery`, which store an answer assembled from several reads under a key of their own, published a connection those reads had not had. Offline the four reads each marked the app stale and the composite write then cleared it, so the editor showed yesterday's schedule with nothing saying so, which is exactly what ApiCache's own doc forbids. Liveness moved to `noteLiveAnswer()`, called by `Axios.get` on success; `writeCache` only writes. A test now asserts that a write cannot claim it. |
| 2026-08-19 | **Comments say what the code is, not what it used to be.** A round of fixes left several files opening with the bug they replaced, worst in `alarmSupport.ts`, which is what everything else asks whether an alarm can ring at all and led with two paragraphs about a deleted comment, naming `get` and `getEnforcing`, which are called nowhere. The rule: keep the sentence that stops the bug recurring, in the imperative and about the present ("a `useFocusEffect` callback re-runs on every focus, so consume the intent rather than test it"), and let this log hold the story. It is dated, which is where a before-and-after stays true. |
| 2026-08-18 | **A refresh is one event, not a mode.** `useNextAlarm` derived "the user asked for this" from `attempt > 0`, a counter that never returns to zero, so after a single press of Refresh every focus of Today took the forced path. That path calls `armSchedule` per active schedule, which is the one call there that spends an NS and a TomTom request, on a screen documented as free to open. The intent now lives in a ref that is cleared when it is read, because `useFocusEffect` re-runs its callback on every focus whether or not a dependency changed, so anything not consumed leaks into the next focus. |
| 2026-08-18 | **Arming is isolated per morning, like every other step around it.** `Promise.all(occurrences.map(arm))` was the only unguarded call in `useNextAlarm`. `schedule` refuses a time in the past, so a single stale occurrence rejected the whole load, dropped the screen to `failed` with no occurrence, and thereby handed over to `armLocally`, replacing a good server answer with a pessimistic local guess. |
| 2026-08-18 | **The ring screen shows translated copy, never an exception message.** It rendered `error.message`, so strings written for whoever debugs this ("The native alarm module is missing from this build.") could appear in English on the one screen a user cannot navigate away from. The raw text goes to the log and the sentence comes from `ring.stop_failed` / `ring.leave_failed`. |
| 2026-08-18 | **Onboarding names its two records from i18n.** `'Weekday'` and `'Work mornings'` were hardcoded. A schedule name is not internal: it shows on the schedules list and travels into the alarm notification body, so a Dutch user was woken by English copy, from the one place a search for strings in JSX would never find it. |
| 2026-08-18 | **The ring screen's fixed palette got names: `Night` in `Stylesheet.ts`.** It stays outside `Colors` because it is not a theme and must not be offerable as one; it is nine values that were hex literals inside `ring.tsx`, which left the preview built to exercise that screen unable to refer to them. |
| 2026-08-18 | **The diagnostics panel keeps only the alarm hardware.** Bundle, API, ring preview, simulation, push and engine each owned their own state and now live in `components/debug/`, taking the screen from 817 lines to 441. What stays is what reads and re-reads one cluster through `refresh`: permissions, native modules, the tone, and the buttons that arm a test alarm. Extracting those would have replaced shared state with a prop list per section, which is the trade that makes a split worse than the file it fixed. |
...
- **Freshness is one global latch, and a mixed outcome resolves by luck.**
  `markStale` settles its own races by keeping the oldest timestamp, so several
  cached reads report the worst of them. `noteLiveAnswer` clears unconditionally,
  which is fine when every endpoint agrees and wrong when they do not: inside a
  `Promise.all` of four reads, one live answer resolving after three cached ones
  wipes their marker and the screen shows stored data with nothing saying so. The
  opposite resolution order reports it correctly, which is what makes this a race
  rather than a bug with a reproduction.

  Only reachable when endpoints disagree about reachability, so it is deliberately
  not fixed yet. Two shapes to choose between when it is: let a clear be refused by
  a stale mark from the same batch, which is small and keeps the latch; or track
  freshness per key and derive the notice from what a screen is actually showing,
  which is honest for a screen reading two endpoints where exactly one is down, and
  needs a notion of "on screen" that this module does not currently have.
